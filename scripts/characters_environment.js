...
function game() {
  ...
  if (gameConfig.status === 'start') {
    ...
    textSize(40);
    textAlign(CENTER);
    text("Presiona cualquier flecha o el boton comenzar", gameConfig.screenX / 2, gameConfig.screenY / 2);
    textSize(40);
  ...
  if (gameConfig.status === 'gameover') {
    ...
    textSize(15);
    text("Presiona espacio o el boton comenzar para iniciar", gameConfig.screenX / 2, gameConfig.screenY / 2 + 135);
    textSize(40);
...
function manualControl(character) {
  ...
  if (character.live) {
    if (keyDown(control.left) || x > 300) {
      character.velocity.x -= gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(-1);
    }

    else if (keyDown(control.right) || x < 200) {
      character.velocity.x += gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(1);
    } else {
...
function jumping(character) {
  if ((keyWentDown(control.up) && character.live) || (touchIsDown && character.live) || y<300) {
...
function scores(character) {
  ...
  text("puntaje: " + round(gameConfig.scores), 20, 40);
